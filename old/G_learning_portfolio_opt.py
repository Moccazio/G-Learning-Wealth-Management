...
class G_learning_portfolio_opt:
    ...
    def __init__(self, 
        ...
        assert Sigma_r.shape[0] == num_risky_assets
        
        self.Sigma_r_np = Sigma_r
        ...
        self.F_xx = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.F_x = torch.zeros(self.num_steps, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.F_0 = torch.zeros(self.num_steps, dtype=torch.float64, requires_grad=False)

        self.Q_xx = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.Q_uu = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.Q_ux = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
        ...
        self.R_xx = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.R_uu = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.R_ux = torch.zeros(self.num_steps, self.num_assets, self.num_assets, dtype=torch.float64, requires_grad=False)
    ...
    def reset_prior_policy(self):
        ...
        self.u_bar_prior = torch.zeros(self.num_steps, self.num_assets, requires_grad=False, dtype=torch.float64)
        self.v_bar_prior = torch.zeros(self.num_steps, self.num_assets, self.num_assets, requires_grad=False, dtype=torch.float64)
        self.Sigma_prior = torch.zeros(self.num_steps, self.num_assets, self.num_assets, requires_grad=False, dtype=torch.float64)
        self.Sigma_prior_inv = torch.zeros(self.num_steps, self.num_assets, self.num_assets, requires_grad=False, dtype=torch.float64)
        ...
        for t in range(self.num_steps):
            ...
            self.Sigma_prior_inv[t, :, :] = 10.0 * torch.eye(self.num_assets).clone()
    ...
    def reward_fun(self, t, x_vals, u_vals, exp_rets, lambd, Sigma_hat):
        ...
        aux_2 = - u_vals.sum()   
        aux_3 = 2 * self.lambd.clone() * p_hat * x_plus.dot(torch.ones(self.num_assets, dtype=torch.float64) + exp_rets)
    ...
    def compute_reward_fun(self):
        ...
        for t in range(self.num_steps):
            ...
            self.R_xx[t, :, :] = (
                -self.lambd.clone() * (self.eta.clone() ** 2) * (self.rho.clone() ** 2) * one_one_T_mat
            ...
            self.R_x[t, :] = (
                -2 * self.lambd.clone() * self.eta.clone() * self.rho.clone() * (1 - self.rho.clone()) * benchmark_portf
    ...
    def project_cash_injections(self):
        ...
        for t in range(1, self.num_steps):
            ...
            self.x_vals[t, :] = (torch.ones(self.num_assets, dtype=torch.float64) + self.exp_returns[t, :]) * self.x_vals[t, :]
            self.expected_c_t[t] = delta_x_t.sum().data
            self.expected_portf_val[t] = self.x_vals[t, :].sum().data
    ...
    def step_G_learning(self, t, err_tol, max_iter):
        ...
        while self.iter_counts[t] < max_iter:
            ...
            tol = torch.max(err_u_bar, err_v_bar)
    ...
    def update_F_params(self, t):
        ...
        self.F_xx[t, :, :] = (
            self.Q_xx[t, :, :].clone()
            + (1 / (2 * self.beta.clone())) * (
                U_t.t().mm(Sigma_p_bar_inv.clone().mm(U_t))
        ...
        self.F_x[t, :] = (
            self.Q_x[t, :].clone()
            + (1 / self.beta.clone()) * (
                U_t.mv(Sigma_p_bar_inv.clone().mv(W_t))
        ...
        self.F_0[t] = (
            self.Q_0[t].clone()
            + (1 / (2 * self.beta.clone())) * (
                W_t.dot(Sigma_p_bar_inv.clone().mv(W_t))
                - self.u_bar_prior[t, :].clone().dot(
                    self.Sigma_prior_inv[t, :, :].clone().mv(self.u_bar_prior[t, :].clone())
                )
            )
            - (1 / (2 * self.beta.clone())) * (
                torch.log(torch.det(self.Sigma_prior[t, :, :].clone() + self.reg_mat))
    ...
    def update_policy_params(self, t):
        ...
        self.u_bar_prior[t, :] = Sigma_prior_new.mv(
            self.Sigma_prior_inv[t, :, :].clone().mv(self.u_bar_prior[t, :].clone())
            + self.beta.clone() * self.Q_u[t, :].clone()
        )
        self.v_bar_prior[t, :, :] = Sigma_prior_new.clone().mm(
            self.Sigma_prior_inv[t, :, :].clone().mm(self.v_bar_prior[t, :, :].clone())
    ...
    def compute_reward_on_traj(self, t, x_t, u_t):
        """
    ...
    def compute_G_fun_on_traj(self, t, x_t, u_t):
        """
        Given time t and corresponding values of vectors x_t, u_t, compute the total reward for this step
        """
    ...
    def compute_F_fun_on_traj(self, t, x_t):
        """
        Given time t and corresponding values of vectors x_t, compute the total reward for this step
        """
    ...
    def MaxEntIRL(self, trajs, learning_rate, err_tol, max_iter):
        """
        ...
        for i in range(num_iter_IRL):
            ...
            for n in range(num_trajs):
                if n % 100 == 0:
                    print(n)
                for t in range(self.num_steps):
                    self.realized_rewards[n, t] = self.compute_reward_on_traj(
                        t, self.data_xvals[n, t, :], self.data_uvals[n, t, :]
                    )
                    self.realized_G_fun[n, t] = self.compute_G_fun_on_traj(
                        t, self.data_xvals[n, t, :], self.data_uvals[n, t, :]
                    )
                    self.realized_F_fun[n, t] = self.compute_F_fun_on_traj(
                        t, self.data_xvals[n, t, :]
                    )
